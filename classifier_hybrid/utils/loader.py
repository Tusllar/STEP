...
def load_data_train(_path, _ftype, joints, coords, cycles=1, test_size=0.0):

    file_affeature = os.path.join(_path, 'affectiveFeatures_3D_train'+_ftype+'.h5')
    ff1 = h5py.File(file_affeature, 'r')
    file_gait = os.path.join(_path, 'Data_train' + _ftype + '.h5')
    ...
    for si in range(num_samples):
        ...
        # data = ff3[list(ff3.keys())[si]][()]
        # print(f"Key: {list(ff3.keys())[si]}")
        # print(f"Data type: {type(data)}")
        # print(f"Data: {data}")
        # Đọc nhãn (dòng cuối cùng trong mảng nhãn)
        raw_label = ff3[ff2_group_key][()]
...
def load_data_test(_path, _ftype, joints, coords, cycles=1, test_size=0):

    file_affeature = os.path.join(_path, 'affectiveFeatures_3D_test'+_ftype+'.h5')
    ff1 = h5py.File(file_affeature, 'r')
    file_gait = os.path.join(_path, 'Data_test' + _ftype + '.h5')
    ff2 = h5py.File(file_gait, 'r')
